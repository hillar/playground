...
class Logger {
  constructor(user,ip,ctx) {
    ...
    for (const method of SEVERITYLOG) {
      this[method.toLowerCase()] = (...msg) => console.log(nowAsJSON(),os.hostname(),'[',process.alias,':',process.pid,']',method.toUpperCase(),':',this.user,this.ip,this.ctx, ...msg)
    }
    for (const method of SEVERITYERROR) {
      this[method.toLowerCase()] = (...msg) => console.error(nowAsJSON(),os.hostname(),'[',process.alias,':',process.pid,']',method.toUpperCase(),':',this.user,this.ip,this.ctx, ...msg)
    }
  }
}
